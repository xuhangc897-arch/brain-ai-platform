...
# 2026-08-23｜首页案件选择手机布局修正

- 手机端案件导航改为单张主卡片展示，隐藏被裁切且不可操作的侧卡；继续保留左右滑动、44px 箭头和点击主卡片进入案件的原逻辑。
- 案件场景只保留紧凑的“重看引导”入口，登录状态、教师端和退出按钮不再覆盖标题；标题、进度、卡片和底部提示重新按安全区分层。
- 当前案件封面在切换后提高加载优先级，并保留进入导航场景后才加载的性能策略。
- 375×812、390×844、430×932 下封面可见、无页面横向溢出；1440×900 下继续显示原五卡桌面轮播。
