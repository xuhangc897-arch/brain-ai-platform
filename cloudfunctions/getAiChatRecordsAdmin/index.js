...
exports.main = async (event) => {
  ...
  const [current, legacy] = await Promise.all([records.where(condition).orderBy("uploadedAt", "desc").limit(limit).get(), legacyRecords.where(legacyCondition).orderBy("uploadedAt", "desc").limit(limit).get()]);
  const combined = (Array.isArray(current.data) ? current.data.map((item) => wrapper(item, "ai_chat_records")) : []).concat((Array.isArray(legacy.data) ? legacy.data : []).flatMap(flattenLegacy));
  ...
  return { ok: true, records: deduped.filter((record) => { const value = timeValue(record.uploadedAt || record.createdAt); return value >= from && value <= to; }).slice(skip, skip + limit) };
};

exports.__test = { wrapper, flattenLegacy, fingerprint };
