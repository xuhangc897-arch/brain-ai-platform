...
function fromLegacy(record) {
  ...
  const results = {}; Object.keys(state).forEach((key) => { if (!["fields", "surveys", "knowledgeQuiz", "knowledgeAssessment", "studentId", "studentName", "studentAge", "className", "groupName", "groupId", "savedAt", "createdAt", "currentStep", "maxUnlockedStep", "flowVersion"].includes(key)) results[key] = state[key]; });
